...
export default function TemplatesPage() {
        ...
        try {
            ...
            setValidation({
                ...
                checks: [{
                    ...
                    status: "KO",
                    detail: "Le serveur backend est inaccessible. V├®rifiez qu'il est d├®marr├®.",
                }],
            });
        } finally {
            setValidating(false);
    ...
    const handleConfirmGenerate = async () => {
        ...
        try {
            ...
            setGenerateError(error instanceof Error ? error.message : "Erreur lors de la g├®n├®ration.");
        } finally {
            setGenerating(null);
        }
    };
            ...
            <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) setValidation(null); }}>
                <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {targetMode === "normal" ? "Liasse Fiscale Compl├¿te (SYSCOHADA)" : "SMT ÔÇö Syst├¿me Minimal de Tr├®sorerie"}
                            {" ÔÇö "}Dossier : <span className="font-medium text-foreground">{activeCompany.name}</span>
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-3 py-2">
                        ...
                        {selectedDocId === "all" && (
                            <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded px-3 py-2">
                                ÔÜá Toutes les ├®critures du dossier seront utilis├®es (cumul g├®n├®ral).
                            </p>
                        )}
                    </div>
                            ...
                            <>
                                ...
                                {validation.blockers.length > 0 && (
                                    <div className="rounded-lg border border-red-300 bg-red-50 px-4 py-3 space-y-1">
                                        <p className="text-xs font-bold text-red-700 uppercase tracking-wide">ÔØî Erreurs bloquantes</p>
                                        {validation.blockers.map((b, i) => (
                                            <p key={i} className="text-xs text-red-700 leading-relaxed">ÔÇó {b}</p>
                                        ))}
                                    </div>
                                )}
                                ...
                                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">D├®tail des v├®rifications</p>
                                ...
                                <Button variant="ghost" size="sm" onClick={runValidation} className="text-xs w-full mt-1">
                                    <RefreshCw className="h-3 w-3 mr-1.5" /> Relancer la v├®rification
                                </Button>
                            </>
                        )}
                    ...
                    <DialogFooter className="gap-2 pt-2">
                        ...
                        >
                            {generating ? (
                                <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> G├®n├®ration...</>
                            ) : (
                                <><FileDown className="mr-2 h-4 w-4" /> G├®n├®rer & T├®l├®charger</>
                            )}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            ...
            <div className="flex flex-col gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-blue-900 dark:text-blue-400">
                        ├ëtats Financiers &amp; Fiscaux
                    </h1>
                    <p className="text-muted-foreground">
                        G├®n├®ration automatique des liasses selon les normes SYSCOHADA R├®vis├® ÔÇö OTR Togo.
                    </p>
                </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
                <Card className="border-l-4 border-l-emerald-600 shadow-md hover:-translate-y-1 transition-transform duration-200">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            Liasse Fiscale Compl├¿te (Normal)
                        </CardTitle>
                        <CardDescription>
                            Mod├¿le officiel OTR 2025. Inclut Bilan, Compte de R├®sultat, TAFIRE et notes annexes.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1 mb-6">
                            <li>Bilan Actif / Passif (Tableaux 1 &amp; 2)</li>
                            <li>Compte de R├®sultat (Tableau 3)</li>
                            <li>Passage R├®sultat Comptable ÔåÆ Fiscal (Tableau 4)</li>
                            <li>Tableau des Flux de Tr├®sorerie (TAFIRE)</li>
                        </ul>
                        <Button
                        ...
                        >
                            {generating === "normal" ? (
                                ...
                                <><FileDown className="mr-2 h-5 w-5" /> G├®n├®rer (.xlsx)</>
                            )}
                        </Button>
                    </CardContent>
                </Card>
                ...
                <Card className="border-l-4 border-l-blue-600 shadow-md hover:-translate-y-1 transition-transform duration-200">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Pour les petites entit├®s. D├®claration simplifi├®e OTR.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1 mb-6">
                            <li>Bilan Simplifi├®</li>
                            <li>Compte de R├®sultat Simplifi├®</li>
                            <li>├ëtat des Flux (simplifi├®)</li>
                        </ul>
                        <Button
                        ...
                        >
                            {generating === "smt" ? (
                                ...
                                <><FileDown className="mr-2 h-5 w-5" /> G├®n├®rer SMT (.xlsx)</>
                            )}
                        </Button>
                    </CardContent>
                </Card>
